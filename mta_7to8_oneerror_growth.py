...
def check_hamming(l1, l2,i,j):
    ...
    x1 = (bin(i)[2:].zfill(7))
    x2 = (bin(j)[2:].zfill(7))
    hdx = sum( c1 != c2 for c1,c2 in zip(x1,x2))
    
    if(hdx > 1):
        s.add(z3.PbGe([(x,1) for x in t], hdx-1)) 
                   

code=[[z3.Bool('code_%d_%d' % (r,c)) for c in range(BITSo)] for r in range(ROWS)]
...
for i in range(ROWS):    
    ...
    for j in range(i):
        c2=[code[i][bit] for bit in range(BITSo)]
        c2=[code[j][bit] for bit in range(BITSo)] 
...
if result==z3.unsat:
    ...
    for i in range(ROWS):
        ...
        for bit in range(n_plus_k):
            t=t+str(bool_to_int(z3.is_true(m[code[i][n_plus_k-1-bit]])))+" "
        print (t)
